...
static const char *const TAG = "improv_thread.component";
...
// Improv service data constants - using custom service ID for Thread
...
// Thread-specific command types (extending Improv protocol)
enum ThreadCommand : uint8_t {
  THREAD_SETTINGS = 0x03,  // New command for Thread dataset
};

ImprovThreadComponent::ImprovThreadComponent() { global_improv_thread_component = this; }

void ImprovThreadComponent::setup() {
...
void ImprovThreadComponent::setup_characteristics() {
...
void ImprovThreadComponent::set_status_indicator_state_(bool state) {
#ifdef USE_OUTPUT
  if (this->status_indicator_ == nullptr)
    return;
...
const char *ImprovThreadComponent::state_to_string_(improv::State state) {
  switch (state) {
    case improv::STATE_STOPPED:
      return "STOPPED";
    case improv::STATE_AWAITING_AUTHORIZATION:
...
bool ImprovThreadComponent::check_identify_() {
  uint32_t now = millis();
...
void ImprovThreadComponent::set_state_(improv::State state, bool update_advertising) {
...
void ImprovThreadComponent::set_error_(improv::Error error) {
...
void ImprovThreadComponent::send_response_(std::vector<uint8_t> &&response) {
  this->rpc_response_->set_value(std::move(response));
  if (this->state_ != improv::STATE_STOPPED)
    this->rpc_response_->notify();
}

void ImprovThreadComponent::start() {
...
void ImprovThreadComponent::stop() {
...
float ImprovThreadComponent::get_setup_priority() const { return setup_priority::AFTER_BLUETOOTH; }

void ImprovThreadComponent::dump_config() {
  ESP_LOGCONFIG(TAG, "Improv Thread:");
#ifdef USE_BINARY_SENSOR
...
void ImprovThreadComponent::process_incoming_data_() {
    ...
    switch (command.command) {
      ...
      case THREAD_SETTINGS: {
        if (this->state_ != improv::STATE_AUTHORIZED) {
          ESP_LOGW(TAG, "Thread settings received, but not authorized");
          this->set_error_(improv::ERROR_NOT_AUTHORIZED);
          this->incoming_data_.clear();
          return;
        }
        
        // For Thread, we expect the dataset TLV in the first string parameter
        if (command.ssid.empty()) {
          ESP_LOGW(TAG, "Thread dataset is empty");
          this->set_error_(improv::ERROR_INVALID_RPC);
          this->incoming_data_.clear();
          return;
        }
        ...
        // Store the dataset
        this->thread_dataset_.clear();
        this->thread_dataset_.insert(this->thread_dataset_.end(), command.ssid.begin(), command.ssid.end());
        
        ESP_LOGD(TAG, "Received Thread dataset TLV (%d bytes)", this->thread_dataset_.size());
        ...
        if (openthread::global_openthread_component != nullptr) {
          ...
          if (error != OT_ERROR_NONE) {
            ESP_LOGE(TAG, "Failed to enable Thread interface: %d", error);
            this->set_error_(improv::ERROR_UNABLE_TO_CONNECT);
            this->incoming_data_.clear();
            return;
          }
          ...
          ESP_LOGD(TAG, "Thread dataset applied, waiting for connection");
        }

        this->set_state_(improv::STATE_PROVISIONING);

        auto f = std::bind(&ImprovThreadComponent::on_thread_connect_timeout_, this);
...
void ImprovThreadComponent::on_thread_connect_timeout_() {
  this->set_error_(improv::ERROR_UNABLE_TO_CONNECT);
  this->set_state_(improv::STATE_AUTHORIZED);
#ifdef USE_BINARY_SENSOR
  if (this->authorizer_ != nullptr)
    this->authorized_start_ = millis();
#endif
  ESP_LOGW(TAG, "Timed out while connecting to Thread network");
}

void ImprovThreadComponent::check_thread_connection_() {
  if (openthread::global_openthread_component == nullptr) {
    return;
  }

  if (!openthread::global_openthread_component->is_connected()) {
    return;
  }
  ...
  if (this->state_ == improv::STATE_PROVISIONING) {
    ...
    // Build URL list
    std::string url_strings[2];
    size_t url_count = 0;

#ifdef USE_IMPROV_THREAD_NEXT_URL
    // Add next_url if configured
...
void ImprovThreadComponent::advertise_service_data_() {
...
void ImprovThreadComponent::update_advertising_type_() {
  uint32_t now = App.get_loop_component_start_time();
...
improv::State ImprovThreadComponent::get_initial_state_() const {
#ifdef USE_BINARY_SENSOR
...
ImprovThreadComponent *global_improv_thread_component =
